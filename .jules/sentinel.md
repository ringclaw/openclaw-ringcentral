...
## 2026-03-06 - Media Upload Path Traversal
**Vulnerability:** The `fetchRemoteMedia` function in both `src/channel.ts` and `src/monitor.ts` returned a `loaded.filename` directly from external sources, which was immediately passed unsanitized into `uploadRingCentralAttachment`. This created a risk for path traversal or malicious file naming.
**Learning:** External API payloads containing filenames must never be trusted blindly. While `sanitizeAttachmentFilename` was created earlier to address similar vulnerabilities, its usage was missed during the implementation of outgoing media uploads.
**Prevention:** Always sanitize filenames returned by external APIs, including those retrieved via methods like `fetchRemoteMedia`, before passing them to internal processing or upload functions. Ensure `sanitizeAttachmentFilename` is applied consistently across all code paths.
